...
## Multi-threaded N-body problem with N threads

Upgrade your assignment from previous day to support features mentioned in this document. 
Don't replace the original code, but commit new directory/files to git with a 
multi-threaded version of the N-body problem.

This threaded version is very different from the Day21 threaded version. In this version
each of the N bodies will become a thread. So you now have N threads if you have N bodies.
Each body (thread) begins it's journey at a certain random coordinate (x, y). And each body
still experiences forces from other N-1 bodies. This means you still need to calculate the
total force vector on a body. Each thread calculates it's own force vector, acceleration,
velocity and position at each timestep. You do not need to worry about sleep here.

You will need to determine the data that needs to be shared across threads. For instance,
while one body computes it's own position, it's position is being modified by the thread
owner for that body. While this position is being modified, another thread cannot access
the position. So the other threads need to wait until the mutex is available. This will
prevent other threads from reading inconsistent data. If you find any more examples of 
concurrent modifications happening, you will need to add them to this list of possible
variables that you will need to manage.

You can picture your N-body problem as N threads which are now simulating the simultaneous
movement of these bodies in time. So all calculations are being performed in parallel
rather than sequentially. How do you think this will improve upon your calculations and why?
Please add your analysis along with the code examples to justify your analysis. You may
want to point out the advantages of using this approach by pointing out the best and worse
case calculations among the N bodies that are being calculated.
